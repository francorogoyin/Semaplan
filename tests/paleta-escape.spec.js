...
test("escape cierra la paleta aunque el input pierda foco", async ({
  page
}) => {
  const estadoInicial = {
    Tareas: [],
  ...
  const overlay = page.locator("#Paleta_Overlay");

  await expect(overlay).toHaveClass(/Activo/);

  await page.evaluate(() => {
    document.getElementById("Paleta_Input")?.blur();
  });

  await page.keyboard.press("Escape");

  await expect(overlay).not.toHaveClass(/Activo/);
});
