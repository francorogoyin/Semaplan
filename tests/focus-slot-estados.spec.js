...
test("usa botones de estado en focus para slots", async ({
  page
}) => {
  ...
  await page.evaluate(({ Fecha, Hora }) => {
    ...
    window.Inicializar();
    Focus_Navegacion_Id = `Slot|${Fecha}|${Hora}`;
    Abrir_Focus_Mode();
  }, { Fecha, Hora });

  const Fila = page.locator(".Focus_Sub_Item").first();
  await expect(Fila.locator("select")).toHaveCount(0);
  await expect(
    Fila.locator(".Abordaje_Planear_Btn")
  ).toHaveCount(1);
  await expect(
    Fila.locator(".Abordaje_Toggle_Btn")
  ).toHaveCount(2);

  await Fila.locator(".Abordaje_Toggle_Btn").nth(1).click();
  let Estado = await page.evaluate(({ Fecha, Hora }) => {
    return Planes_Slot[`${Fecha}|${Hora}`].Items[0].Estado;
  }, { Fecha, Hora });
  expect(Estado).toBe("Abordado");

  await Fila.locator(".Abordaje_Toggle_Btn").nth(0).click();
  Estado = await page.evaluate(({ Fecha, Hora }) => {
    return Planes_Slot[`${Fecha}|${Hora}`].Items[0].Estado;
  }, { Fecha, Hora });
...
test("congela countdown del slot vencido y pinta cierre", async ({
  ...
  await page.addInitScript(({ Estado, Ahora }) => {
    const Fecha_Fija = new Date(Ahora).valueOf();
  ...
  await expect(page.locator("#Focus_Countdown_Numero"))
    .toHaveText("—");
  await expect(page.locator("#Focus_Countdown_Etiqueta"))
    .toHaveText("Pendiente");
  await expect(page.locator("#Focus_Countdown_Card"))
    .toHaveClass(/Vencido/);

  await page.locator(".Abordaje_Toggle_Btn").first().click();
