...
test("reordena notas dentro del cajón", async ({ page }) => {
  const estadoInicial = {
    Objetivos: [],
    Eventos: [],
    Metas: [],
  ...
  await page.evaluate(() => {
    ...
    Data.setData("application/x-archivero-nota", "n2");
    Origen.dispatchEvent(new DragEvent("dragstart", {
      dataTransfer: Data,
      bubbles: true
    }));
    const Rect = Destino.getBoundingClientRect();
    Destino.dispatchEvent(new DragEvent("dragover", {
      dataTransfer: Data,
      bubbles: true,
      clientY: Rect.top + 1
    }));
  ...
  const textos = await page.locator(".Archivero_Nota_Texto")
    .allTextContents();
  await expect(
    page.locator("#Archivero_Orden_Select")
  ).toHaveValue("Personalizado");
  expect(textos[0]).toBe("Segunda");
  expect(textos[1]).toBe("Primera");
});
