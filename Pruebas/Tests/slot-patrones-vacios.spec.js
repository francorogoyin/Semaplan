...
test("incluye patrones de slot vacío en slots muertos", async ({
  page
}) => {
  const estadoInicial = {
    Objetivos: [],
  ...
  const nombres = await page.evaluate(() => {
    ...
    window.Inicializar();
    return Obtener_Patrones_Slot("Sueno")
      .map((patron) => patron.Nombre);
  });

  expect(nombres).toContain("Genérico");
  expect(nombres).toContain("Sueño");
});
