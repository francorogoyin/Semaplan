...
const { Iniciar_Servidor_Local } = require("./Servidor_Local");
...
function Configurar_Navegacion(Ventana) {
  Ventana.webContents.setWindowOpenHandler(({ url }) => {
    if (Es_Url_Local(url)) return { action: "allow" };
...
app.whenReady().then(Arrancar_App).then(() => {
  app.on("activate", () => {
...
app.on("before-quit", Cerrar_Servidor_Local);
