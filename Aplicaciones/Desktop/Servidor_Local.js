...
function Resolver_Ruta_Local(Raiz_App, Url_Solicitada) {
  ...
  if (Ruta_Relativa.startsWith("..") || Path.isAbsolute(Ruta_Relativa)) {
    return null;
  }
...
function Crear_Manejador(Raiz_App) {
  return async function Atender(Pedido, Respuesta) {
    try {
      ...
      const Estado = await Fs.promises.stat(Ruta_Local).catch(() => null);
      ...
      Respuesta.end(await Fs.promises.readFile(Ruta_Local));
    } catch (Error) {
...
async function Iniciar_Servidor_Local({ Puerto_Preferido = 4173, Raiz_App }) {
...
module.exports = { Iniciar_Servidor_Local };
